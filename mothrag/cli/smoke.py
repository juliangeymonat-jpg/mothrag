...
def main(argv: list[str] | None = None):
    ap = argparse.ArgumentParser(
        prog="mothrag-smoke",
        description="MothRAG smoke test on a HotpotQA-style subset.",
    )
    ap.add_argument(
        "--corpus",
        required=True,
    ...
    ap.add_argument(
        "--max", type=int, default=5, help="Number of questions to evaluate (default 5)."
    )
    ap.add_argument(
        "--reader",
    ...
    print(
        f"Running MothRAG ({args.config}) on {len(qtexts)} questions with reader={args.reader} ...",
        file=sys.stderr,
    )
    ...
    for q, r in zip(selected, results):
        ...
        f1_total += f1
        print(
            f"  [{q.get('_id', '?')}] EM={em:.0f} F1={f1:.2f}  pred={r.answer!r}  gold={gold!r}  ({r.latency_s:.2f}s)"
        )
